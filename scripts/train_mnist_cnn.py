#!/usr/bin/env python3
"""
Train a simple CNN on MNIST and extract embeddings at various training stages.

This script:
1. Trains a simple CNN classifier on MNIST
...
Output:
    static/data/mnist-training-embeddings.json
"""
...
class SimpleCNN(nn.Module):
    ...
    Architecture:
        ...
        Flatten -> Linear(1600, 128) -> ReLU  <-- embeddings extracted here
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 64 * 7 * 7)
        x = self.dropout(x)
        x = F.relu(self.fc1(x))  # This is our embedding layer
        x = self.fc2(x)
        return x
...
def extract_embeddings(model, data_loader, device):
    ...
    # MNIST normalization parameters used in transform
    mean = 0.1307
    std = 0.3081
    ...
    with torch.no_grad():
        for data, target in data_loader:
            ...
            # Denormalize and convert back to 0-255 range
            img_data = data.cpu().numpy().squeeze()
            img_denorm = img_data * std + mean  # Reverse normalization
            img_uint8 = np.clip(img_denorm * 255, 0, 255).astype(np.uint8)
            images.append(img_uint8)

    embeddings = np.vstack(embeddings)
    labels = np.concatenate(labels)
    images = np.vstack([img.reshape(-1, 784) for img in images])
...
def main():
    ...
    BATCH_SIZE = 64
    EPOCHS = 10
    VIZ_SAMPLES = 1000  # Number of samples to visualize (100 per digit)
    ...
    viz_subset = Subset(train_dataset, viz_indices)
    ...
    # Capture: before training, after epochs 1, 2, 3, 5, 7, 10
    snapshot_epochs = [0, 1, 2, 3, 5, 7, 10]
    ...
    # Write to file
    output_path = Path(__file__).parent.parent / "static" / "data" / "mnist-training-embeddings.json"
